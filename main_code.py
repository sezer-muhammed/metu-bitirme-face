...
for video in videos:
  ...
  cam = cv2.VideoCapture(video) #Video objesini açar

  fps = 2.5#cam.get(cv2.CAP_PROP_FPS)
  w = int(cam.get(cv2.CAP_PROP_FRAME_WIDTH))
  h = int(cam.get(cv2.CAP_PROP_FRAME_HEIGHT)) #Video bilgilerini çeker, PC'de video kaydı için demoda olmayacak bunlar

  saver = cv2.VideoWriter(f"runs/{video.split('.')[0]}_filled.mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h)) #video kaydı için class oluşumu
  counter = 0 #debug için counter önemsiz demo için
  logger.info(f"====={video}========") 
  ...
  while True:
    ...
    manager.Detect(frame_rgb) #Obje tespiti
